...
export default function Home() {
  ...
  // Scroll to top of chat section after analysis (not bottom of page)
  useEffect(() => {
    if (lastAnalysis && chatSectionRef.current) {
      chatSectionRef.current.scrollIntoView({ behavior: "smooth", block: "start" });
    }
  }, [lastAnalysis]);

  // Only scroll to bottom when new chat messages arrive
  useEffect(() => {
    ...
    try {
      if (igUsername) {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/analyze-profile`, {
          ...
          body: JSON.stringify({ profile: igUsername, focus: "growth, best performing posts, current trends, improvement suggestions" }),
        });
      ...
      if (xUsername) {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/analyze-profile`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ profile: xUsername, focus: "growth, best performing posts, current trends, improvement suggestions" }),
        });
        const data = await res.json();
      ...
      if (fbPage) {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/analyze-profile`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ profile: fbPage, focus: "growth, best performing posts, current trends, improvement suggestions" }),
        });
        const data = await res.json();
      ...
      console.error(error);
      alert("Failed to analyze accounts. Make sure the backend is running.");
    }
  ...
  const handleAnalyzePosts = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/analyze-posts`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ shortcodes: shortcodeList, focus: "engagement comparison, best performing, improvement suggestions" }),
      });
      ...
      if (data.success) {
        append({ role: "user", content: `Here is the analysis for posts: ${shortcodeList.join(", ")}` });
      }
    } catch (error) {
      console.error(error);
    ...
    <main style={{ minHeight: "100vh", backgroundColor: "#000000", color: "#ffffff", padding: "24px", fontFamily: "monospace" }}>
      ...
      <section style={{ border: "2px solid #27272a", padding: "24px", marginBottom: "32px", backgroundColor: "#09090b" }}>
        ...
        <div style={{ display: "flex", gap: "12px", marginBottom: "20px", borderBottom: "1px solid #27272a", paddingBottom: "16px" }}>
          <button onClick={() => setMode("accounts")} style={{ padding: "10px 24px", background: mode === "accounts" ? "#fff" : "#18181b", color: mode === "accounts" ? "#000" : "#fff", border: "1px solid #27272a", fontWeight: "bold", borderRadius: "6px" }}>📊 Compare Accounts</button>
          <button onClick={() => setMode("posts")} style={{ padding: "10px 24px", background: mode === "posts" ? "#fff" : "#18181b", color: mode === "posts" ? "#000" : "#fff", border: "1px solid #27272a", fontWeight: "bold", borderRadius: "6px" }}>📍 Compare Instagram Posts</button>
        </div>
        ...
        {mode === "accounts" ? (
          ...
          <div>
            <label style={{ display: "block", fontSize: "11px", color: "#a1a1aa", marginBottom: "8px" }}>INSTAGRAM REEL URLS OR SHORTCODES</label>
            <input type="text" placeholder="https://www.instagram.com/reel/DZNdBTogxVy/ or DZNdBTogxVy" value={postInput} onChange={(e) => setPostInput(e.target.value)} style={{ width: "100%", padding: "14px", background: "#000", border: "1px solid #27272a", color: "white", marginBottom: "12px" }} />
            <p style={{ fontSize: "12px", color: "#a1a1aa" }}>You can paste full URLs or shortcodes (comma separated)</p>
          </div>
        )}

        <button onClick={mode === "accounts" ? handleAnalyzeAccounts : handleAnalyzePosts} disabled={isProcessing || (mode === "accounts" ? (!igUsername && !xUsername && !fbPage) : !postInput.trim())} style={{ marginTop: "20px", padding: "14px 32px", background: "#fff", color: "#000", fontWeight: "bold", border: "none" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "32px" }}>
        ...
        <section ref={chatSectionRef} style={{ border: "2px solid #27272a", backgroundColor: "#09090b", height: "720px", display: "flex", flexDirection: "column" }}>
          ...
          <form onSubmit={handleSubmit} style={{ padding: "16px", borderTop: "1px solid #27272a", display: "flex", gap: "8px" }}>
            <input type="text" value={input} onChange={handleInputChange} placeholder="Ask anything about performance, trends, or strategy..." style={{ flex: 1, padding: "12px", background: "#000", border: "1px solid #27272a", color: "white" }} />
            <button type="submit" style={{ padding: "0 24px", background: "#fff", color: "#000", fontWeight: "bold" }}>SEND</button>
          </form>
        </section>
      </div>

      {/* Quick Insights - Now below the analysis box on the right */}
      <div style={{ marginTop: "32px", border: "2px solid #27272a", padding: "20px", backgroundColor: "#09090b" }}>
        <h3 style={{ marginBottom: "12px", fontSize: "14px", color: "#ffffff" }}>QUICK INSIGHTS</h3>
